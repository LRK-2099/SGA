...
const senatorApi = api.injectEndpoints({
    endpoints: (builder) => ({
      ...
      getSenators: builder.query({
        // The /senators is grabbing the backend api, so in our backend,
        // we grab the senator data by localhost:3000/api/senators,
        // so we need to put /senators
        query: () => '/senators',  // <-- Removed parentheses here
